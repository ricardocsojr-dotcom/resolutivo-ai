...
Arquivos relevantes:
- a automação provida pelo motor LangGraph — pipeline principal (classifica, filtra histórico, chama IA)
- a automação provida pelo motor LangGraph — gera planilha de exemplo quando não há planilha real
...
### Verificar se há planilha do dia
...
de teste (a automação provida pelo motor LangGraph).
...
- Se houver erro de autenticação na API Gemini, exiba o erro exato e sugira
  verificar a chave `API_KEY` em a automação provida pelo motor LangGraph.
