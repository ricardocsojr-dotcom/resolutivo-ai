...
O documento deve parecer escrito de uma única vez — unitário, limpo, sem colagem de peças
antigas ("Frankenstein jurídico").

## 2. Núcleo universal — aplica-se a toda peça, sem exceção

| Regra | Como aplicar |
|---|---|
| Abertura fixa | "[PARTE EM CAIXA ALTA], já qualificada nos autos, por seus advogados que esta subscrevem, vem, respeitosamente, [à presença de/perante] Vossa Excelência, com fundamento no [dispositivo], [verbo da peça] [NOME DA PEÇA em caixa alta e negrito], pelas razões a seguir expostas." O fundamento legal entra nesse primeiro período — nunca depois. |
...
| Jurisprudência com aterrissagem | Frase introdutória que já anuncia a tese → ementa literal entre aspas/bloco → referência completa (tribunal, número, relator, órgão, data) → parágrafo seguinte reconectando ao caso concreto. Nunca ementa solta. Nunca inventada: toda jurisprudência deve ser verificada no site oficial do tribunal antes de citar. |
...
| Precisão factual | Datas por extenso, número de ID/evento. Nunca aproximação vaga. |
| Desqualificação por vagueza | Ao invalidar tese adversa: "não identifica", "não demonstra", "limitou-se a afirmar genericamente" — nunca adjetivação retórica. |

### Sinais de cadência robótica a evitar já na redação (versão condensada do checklist-3 do revisor)
...
| Tipo de peça | Títulos | Numeração de parágrafo | Sumário executivo | Jurisprudência | Camada adicional |
|---|---|---|---|---|---|
| Manifestação simples (juntada, desinteresse em conciliação, oposição a julgamento) | Nenhum | Não | Não | Só se estritamente necessário | — |
| Manifestação com títulos (prescrição, dilação de prazo, taxatividade) | Caixa alta, sem numeração romana | Opcional | Não | Sim, quando fundamenta o pedido | — |
| Recursal extenso (agravo, apelação, agravo interno, contrarrazões) | Caixa alta + romanos (I, II, III) | Sim, contínua do início ao fim | Obrigatório acima de 15 páginas | Sim, sempre com aterrissagem | Seção de tempestividade/cabimento/preparo no início |
| Peça narrativa (dano moral e afins) | Caixa alta + romanos | Sim | Conforme extensão | Sim | `dano-moral-rct/references/estilo-rct.md`: contraste aparência/realidade, frase-veredito, gradação de gravidade — ler depois deste núcleo, nunca em vez dele |

Regra prática: comece sempre pelo núcleo. A tabela só diz o que **acrescentar** por gênero —
nunca contradiz o núcleo.

## 4. Higienização vocabular — proibidos absolutos

| Expressão proibida | Substituição |
|---|---|
| "o mesmo / a mesma" (como pronome) | "ele", "este" ou retome o termo |
| "junto a" (para órgãos/juízos) | "perante", "ao", "no" |
| "através de" (como meio/instrumento) | "por meio de" ou "mediante" |
| "cumpre salientar" / "imperioso notar" / "por derradeiro" / "salvo melhor juízo" / "vale lembrar" / "é crucial destacar" / "em suma" / "convém registrar" / "impende destacar" / "não é demais lembrar" / "digno de nota" / "vale consignar" | eliminar |
| "nobre julgador" / "digníssimo" / "DD" / "ilustríssimo" | eliminar |
| Advérbios longos em "-mente" em excesso | reformular a frase |
| Adjetivação retórica ("conduta abominável") | descrever o fato objetivamente |
| Latim/estrangeirismo desnecessário | preferir o português; quando indispensável, sempre em *itálico* |

## 5. Formatação institucional
...
- [ ] Toda jurisprudência tem frase de entrada + ementa + referência + aterrissagem?
