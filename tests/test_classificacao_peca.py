...
def test_vault_automatico_is_blocked_without_cerebro_origem() -> None:
...
def test_vault_automatico_is_blocked_for_level_c_even_with_cerebro_origem() -> None:
    report = validate_piece_contract(
        {"nivel_peca": "C", "vault_automatico": True, "vault_automatico_origem": "cerebro-ricar"}
    )
    assert report["status"] == "BLOCK"
    assert any(item["id"] == "vault_automatico_proibido" for item in report["findings"])


def test_vault_automatico_is_allowed_for_cerebro_ricar_in_b_or_a() -> None:
    report = validate_piece_contract(
        {
            "nivel_peca": "B",
            "exigir_esqueleto": True,
            "vault_automatico": True,
            "vault_automatico_origem": "cerebro-ricar",
        }
    )
    assert report["status"] == "PASS"
...
if __name__ == "__main__":
    ...
    test_vault_automatico_is_blocked_without_cerebro_origem()
    test_vault_automatico_is_blocked_for_level_c_even_with_cerebro_origem()
    test_vault_automatico_is_allowed_for_cerebro_ricar_in_b_or_a()
