...
def test_claude_recebe_pacote_por_stdin_e_devolve_resultado_estruturado(tmp_path):
    ...
    with patch.object(MODULE.subprocess, "run") as run:
        ...
        run.return_value.stderr = ""
        run.return_value.stdout = json.dumps(
            {"type": "result", "subtype": "success", "result": "validação", "session_id": "sessao-1", "modelUsage": {"claude-sonnet-5": {"costUSD": 0.01}}},
            ensure_ascii=False,
        )
    ...
    assert result["model_ids"] == ["claude-sonnet-5"]


def test_executor_registra_saida_no_manifesto_quando_recebe_papel(tmp_path):
    ORCHESTRATOR.inicializar_execucao(tmp_path, "caso-123", "A")
    ...
    for phase in ("vault_context_ready", "sources_ready", "council_ready", "skeleton_ready", "awaiting_skeleton_approval"):
